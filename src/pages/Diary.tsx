...
export function DiaryPage() {
  ...
  const phases: { id: Phase; title: string; sub: string; img: string }[] = [
    { id: 'morning', title: 'Morning', sub: 'Sleep ┬À dreams ┬À practice', img: IMAGES.dawn },
    { id: 'day', title: 'Day', sub: 'Conduct ┬À defects ┬À service', img: IMAGES.day },
    { id: 'night', title: 'Night', sub: 'Meditation ┬À retrospect', img: IMAGES.night },
    { id: 'summary', title: 'Summary', sub: 'Awareness ┬À energy', img: IMAGES.sum },
  ];
  ...
  return (
    <Shell>
              ...
              <div key={d.id} className="gx-card" style={{ background: '#fff', marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <div className="gx-btn-row">
                    <button className="gx-btn gx-btn-ghost" onClick={() => set(ck, Math.max(0, Number(entry[ck] || 0) - 1) as never)}>ÔêÆ</button>
      ...
      <div className="gx-btn-row" style={{ marginTop: 14 }}>
        <button className="gx-btn gx-btn-primary" onClick={() => toast(`Saved ┬À ${active}`)}>Lock entry</button>
      </div>
    </Shell>
  );
}
